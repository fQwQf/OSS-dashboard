...
本项目旨在专注于监控 **华中科技大学开放原子开源俱乐部 (`hust-open-atom-club`)** 的活动，并提供 **SIG (Special Interest Group)** 和组织总览的精细化统计。
...
在的 PostgreSQL 实例中创建一个新的数据库，例如 `oss_dashboard`。
...
进入 `backend` 目录，并将 `.env.example` 复制为 `.env` 文件，并根据环境进行修改。
...
**注意:**
*   需要一个 **GitHub Personal Access Token** (`GITHUB_TOKEN`)。该 Token 必须具有访问 `hust-open-atom-club` 组织和其仓库的权限。
*   请确保 `DB_` 和 `REDIS_` 相关的配置与本地服务一致。
...
**启动提示:** 后端服务启动时，如果发现数据库中没有数据，将自动触发 **历史数据回填**。由于引入了 **API 延迟机制**（每次 GitHub API 调用间隔 1 秒），回填过程会较慢。
...
*   **SIG 趋势图:** 用户可以选择一个 SIG，下方会展示该 SIG 聚合后的活动趋势图，包括 **PR/Issue 统计** 和 **Commit 统计**。
*   **数据卡片:** 顶部的活动快照卡片现在展示的是 **组织聚合** 的最新数据。
